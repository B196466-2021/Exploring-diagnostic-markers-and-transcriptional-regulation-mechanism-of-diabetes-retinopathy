...
usage<-function(spec){
	cat("This script is used to plot difference of two group with multiple factors.\n",
getopt(spec,usage=TRUE),
"Options:
	-d, --data	file of data matrix (each row is a factor).
	-g, --group	group information file (Sample<tab>Group).
	-t, --type	plot type: boxplot, violin, boxviolin (default boxplot).
	-m, --method	differential test method: wilcox.test, kruskal.test or t.test (default wilcox.test).
	-c, --color	colors or color palette for group (e.g. 'brown3,dodgerblue3' or ggsci: 'npg', 'aaas', 'nejm', 'lancet', 'jama', 'jco', 'ucscgb', 'd3', 'locuszoom', 'igv', 'uchicago', 'startrek', 'tron', 'futurama', 'rickandmorty', 'gsea', 'ucscgb' or 'simpsons' (default 'npg')).
	-w, --width	width of the out picture (default 7).
	-e, --height	height of the out picture (default 7).
	-x, --xlab	xlab (default NULL).
	-y, --ylab	ylab (default NULL).
	-l, --log	translate value by log2 or not (default FALSE).
	-a, --angle	text angle of x axis (default 0).
	-s, --switch	switch the differential p.value to asterisk or not (default FALSE).
	-r, --errorbar	plot errorbar in boxplot or not (default FALSE).
	-j, --jdt	only do difference test or not (default FALSE).
...
if (is.null(opt$data) || is.null(opt$group) || !is.null(opt$help)) {usage(spec)}
if (is.null(opt$type)) {opt$type='boxplot'}
if (is.null(opt$method)) {opt$method='wilcox.test'}
if (is.null(opt$color)) {opt$color='npg'}
if (is.null(opt$width)) {opt$width=7}
if (is.null(opt$height)) {opt$height=7}
if (is.null(opt$angle)) {opt$angle=0}
if (is.null(opt$jdt)) {opt$jdt=F}
if (is.null(opt$switch)) {opt$switch=F}
if (is.null(opt$log)) {opt$log=F}
if (is.null(opt$out)) {opt$out='./'}
if (is.null(opt$errorbar)) {opt$errorbar=F}
